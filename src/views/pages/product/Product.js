...
const Product = () => {
  ...
  const handleChangeCategory = async (event) => {
    ...
    await axios
      .get(`http://192.168.1.4:9000/api/product/filter?kategori=${category}`)
      .then((response) => {
        setData(response.data);
      });
  ...
  const handleChangeStatus = async (event) => {
    ...
    await axios
      .get(`http://192.168.1.4:9000/api/product/filter?status=${status}`)
      .then((response) => {
        setData(response.data);
      });
    setIsActive(status);
  ...
  const handleDelete = async (_id, e) => {
    try {
      await axios.delete(`http://192.168.1.4:9000/api/product/${_id}`);
